## Hi there 👋
# Olá, eu sou JOÃO NOGUEIRA CLEMENTE 👋

### 🚀 Engenheiro de Energias | Cientista de Dados | Entusiasta de IA

Atualmente, estou em uma jornada de transição de carreira, unindo minha base analítica da **Engenharia de Energias** com o poder da **Inteligência Artificial** e **Data Science**. Sou pós-graduando em **IA Aplicada à Indústria** no UniSENAI e dedico meus dias a construir soluções inteligentes que resolvem problemas reais.

---

### 🛠️ No que estou trabalhando agora:
- 🤖 **Projeto Licita-Hunter**: Uma ferramenta em Python com multiprocessing para análise e classificação automática de editais de licitação.
- ⚖️ **Automador Jurídico**: Aplicação Streamlit que utiliza LangChain e Gemini 1.5 Flash para sumarizar documentos legais.
- 📅 **Desafio 11 Meses de Código**: Comprometido em programar todos os dias (desde Fev/2026) para acelerar minha evolução em engenharia de dados e ML.

---

### 🧰 Tech Stack:

* **Linguagens:** Python, SQL
* **IA & LLMs:** LangChain, RAG, AI Agents, Google Gemini API
* **Data & Web:** Streamlit, Pandas, NumPy
* **Ferramentas:** Git/GitHub, VS Code, Pop!_OS, Docker

---

### 📊 Estatísticas do GitHub:

![Status do meu GitHub](https://github-readme-stats.vercel.app/api?username=Docas32&show_icons=true&theme=radical)
![Linguagens mais usadas](https://github-readme-stats.vercel.app/api/top-langs/?username=Docas32&layout=compact&theme=radical)

---

### 📫 Como me encontrar:

* **LinkedIn:** [www.linkedin.com/in/joão-nogueira-clemente-2327121a5]
* **Portfólio:** Confira meus repositórios para ver o **Licita-Hunter** e o **Removedor de Fundo de Imagens** em ação!

---
*"A entropia sempre cresce, mas o código a gente organiza."* 🌌

<!--
**Docas32/Docas32** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
